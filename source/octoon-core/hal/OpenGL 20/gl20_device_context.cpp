...
namespace octoon
{
	namespace hal
	{
		...
		void
		GL20DeviceContext::setStencilCompareMask(StencilFaceFlags face, std::uint32_t mask) noexcept
		{
		...
		std::uint32_t
		GL20DeviceContext::getStencilCompareMask(StencilFaceFlags face) noexcept
		{
		...
		void
		GL20DeviceContext::setStencilReference(StencilFaceFlags face, std::uint32_t reference) noexcept
		{
			if (face & StencilFaceFlagBits::FrontBit)
			{
		...
		std::uint32_t
		GL20DeviceContext::getStencilReference(StencilFaceFlags face) noexcept
		{
			assert(face == StencilFaceFlagBits::FrontBit || face == StencilFaceFlagBits::BackBit);
		...
		void
		GL20DeviceContext::setStencilWriteMask(StencilFaceFlags face, std::uint32_t mask) noexcept
		{
			if (face & StencilFaceFlagBits::FrontBit)
			{
		...
		std::uint32_t
		GL20DeviceContext::getStencilWriteMask(StencilFaceFlags face) noexcept
		{
			assert(face == StencilFaceFlagBits::FrontBit || face == StencilFaceFlagBits::BackBit);
		...
		GL20DeviceContext::setRenderPipeline(const GraphicsPipelinePtr& pipeline) noexcept
		{
			...
			if (pipeline)
			{
				...
				if (_pipeline != glpipeline)
				{
					...
					auto glprogram = pipelineDesc.getProgram()->downcast_pointer<GL20Program>();
		...
		void
		GL20DeviceContext::setIndexBufferData(const GraphicsDataPtr& data, std::intptr_t offset, IndexFormat indexType) noexcept
		{
			if (data)
			{
				assert(data->isInstanceOf<GL20GraphicsData>());
		...
		void
		GL20DeviceContext::generateMipmap(const std::shared_ptr<GraphicsTexture>& texture) noexcept
		{
			assert(texture);
		...
		GL20DeviceContext::clearFramebuffer(std::uint32_t i, ClearFlags flags, const float4& color, float depth, std::int32_t stencil) noexcept
		{
			...
			if (mode != 0)
			{
				...
				if (!depthWriteEnable && flags & ClearFlagBits::DepthBit)
				{
		...
		void
		GL20DeviceContext::discardFramebuffer(const GraphicsFramebufferPtr& src, ClearFlags flags) noexcept
		{
		}
		...
		void
		GL20DeviceContext::readFramebuffer(std::uint32_t i, const std::shared_ptr<GraphicsTexture>& texture, std::uint32_t miplevel, std::uint32_t x, std::uint32_t y, std::uint32_t width, std::uint32_t height) noexcept
		{
		...
		void
		GL20DeviceContext::readFramebufferToCube(std::uint32_t i, std::uint32_t face, const std::shared_ptr<GraphicsTexture>& texture, std::uint32_t miplevel, std::uint32_t x, std::uint32_t y, std::uint32_t width, std::uint32_t height) noexcept
		{
			GLenum internalFormat = GL20Types::asTextureFormat(texture->getTextureDesc().getTexFormat());
			if (internalFormat == GL_INVALID_ENUM)
			{
		...
		GraphicsDevicePtr
		GL20DeviceContext::getDevice() const noexcept
		{
			return _device.lock();
		}
	}
