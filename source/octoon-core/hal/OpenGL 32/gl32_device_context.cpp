...
namespace octoon
{
	namespace hal
	{
		...
		void
		GL32DeviceContext::setViewport(std::uint32_t i, const math::float4& view) noexcept
		{
		...
		void
		GL32DeviceContext::setScissor(std::uint32_t i, const math::uint4& scissor) noexcept
		{
			assert(_glcontext->getActive());
		...
		void
		GL32DeviceContext::setStencilCompareMask(StencilFaceFlags face, std::uint32_t mask) noexcept
		{
		...
		std::uint32_t
		GL32DeviceContext::getStencilCompareMask(StencilFaceFlags face) noexcept
		{
		...
		void
		GL32DeviceContext::setStencilReference(StencilFaceFlags face, std::uint32_t reference) noexcept
		{
			if (face & StencilFaceFlagBits::FrontBit)
			{
		...
		std::uint32_t
		GL32DeviceContext::getStencilReference(StencilFaceFlags face) noexcept
		{
			assert(face == StencilFaceFlagBits::FrontBit || face == StencilFaceFlagBits::BackBit);
		...
		void
		GL32DeviceContext::setStencilWriteMask(StencilFaceFlags face, std::uint32_t mask) noexcept
		{
			if (face & StencilFaceFlagBits::FrontBit)
			{
		...
		std::uint32_t
		GL32DeviceContext::getStencilWriteMask(StencilFaceFlags face) noexcept
		{
			assert(face == StencilFaceFlagBits::FrontBit || face == StencilFaceFlagBits::BackBit);
		...
		GL32DeviceContext::setRenderPipeline(const GraphicsPipelinePtr& pipeline) noexcept
		{
			...
			if (pipeline)
			{
				...
				if (_pipeline != glpipeline)
				{
					...
					auto glprogram = pipelineDesc.getProgram()->downcast_pointer<GL32Program>();
		...
		void
		GL32DeviceContext::setIndexBufferData(const GraphicsDataPtr& data, std::intptr_t offset, IndexFormat indexType) noexcept
		{
			if (data)
			{
				assert(data->isInstanceOf<GL32GraphicsData>());
		...
		void
		GL32DeviceContext::generateMipmap(const std::shared_ptr<GraphicsTexture>& texture) noexcept
		{
			assert(texture);
		...
		void
		GL32DeviceContext::clearFramebuffer(std::uint32_t i, ClearFlags flags, const math::float4& color, float depth, std::int32_t stencil) noexcept
		{
			assert(_glcontext->getActive());
		...
		void
		GL32DeviceContext::blitFramebuffer(const GraphicsFramebufferPtr& src, const math::float4& v1, const GraphicsFramebufferPtr& dest, const math::float4& v2) noexcept
		{
			assert(src);
			...
			this->_readFramebuffer = src ? src->downcast_pointer<GL32Framebuffer>() : nullptr;
			this->_drawFramebuffer = dest ? dest->downcast_pointer<GL32Framebuffer>() : nullptr;
		...
		void
		GL32DeviceContext::discardFramebuffer(const GraphicsFramebufferPtr& framebuffer, ClearFlags flags) noexcept
		{
			assert(framebuffer);
			assert(_glcontext->getActive());
			...
			if (framebuffer)
			{
				auto fbo = framebuffer->downcast_pointer<GL32Framebuffer>();
		...
		void
		GL32DeviceContext::readFramebuffer(std::uint32_t i, const std::shared_ptr<GraphicsTexture>& texture, std::uint32_t miplevel, std::uint32_t x, std::uint32_t y, std::uint32_t width, std::uint32_t height) noexcept
		{
		...
		void
		GL32DeviceContext::readFramebufferToCube(std::uint32_t i, std::uint32_t face, const std::shared_ptr<GraphicsTexture>& texture, std::uint32_t miplevel, std::uint32_t x, std::uint32_t y, std::uint32_t width, std::uint32_t height) noexcept
		{
			GLenum internalFormat = GL32Types::asTextureFormat(texture->getTextureDesc().getTexFormat());
			if (internalFormat == GL_INVALID_ENUM)
			{
		...
		GL32DeviceContext::initStateSystem() noexcept
		{
			...
			_viewports.resize(systemInfo.maxViewports, math::float4(0, 0, 0, 0));
			_scissors.resize(systemInfo.maxViewports, math::uint4(0, 0, 0, 0));
		...
		GraphicsDevicePtr
		GL32DeviceContext::getDevice() const noexcept
		{
			return _device.lock();
		}
	}
