...
namespace octoon
{
	namespace hal
	{
		...
		void
		GL32GraphicsAttribute::setSemantic(std::string_view semantic) noexcept
		{
			_semantic = semantic;
		}
		...
		void
		GL32GraphicsUniform::setName(std::string_view name) noexcept
		{
			_name = name;
		}
		...
		void
		GL32GraphicsUniform::setSamplerName(std::string_view name) noexcept
		{
			_samplerName = name;
		}
		...
		void
		GL32GraphicsUniform::setType(UniformAttributeFormat type) noexcept
		{
			_type = type;
		}
		...
		void
		GL32GraphicsUniform::setShaderStageFlags(ShaderStageFlags flags) noexcept
		{
			_stageFlags = flags;
		}
		...
		bool
		GL32Shader::HlslCodes2GLSL(ShaderStageFlags stage, const std::string& codes, std::string& out)
		{
		...
		bool
		GL32Shader::HlslByteCodes2GLSL(ShaderStageFlags stage, const char* codes, std::string& out)
		{
#if defined(OCTOON_BUILD_PLATFORM_WINDOWS) && defined(OCTOON_FEATURE_HAL_USE_HLSL)
		...
		GL32Shader::getShaderDesc() const noexcept
		{
			return _shaderDesc;
		}
		...
		UniformAttributeFormat
		GL32Program::toGraphicsUniformType(std::string_view name, GLenum type) noexcept
		{
		...
		GL32Program::getProgramDesc() const noexcept
		{
			return _programDesc;
		}
