...
namespace octoon
{
	namespace hal
	{
		...
		GL45Framebuffer::bindRenderTexture(GraphicsTexturePtr renderTexture, GLenum attachment, GLint level, GLint layer) noexcept
		{
			...
			if (textureDesc.getTexDim() == TextureDimension::Texture2DArray ||
				textureDesc.getTexDim() == TextureDimension::Texture2DArrayMultisample ||
		...
		const std::uint64_t
		GL45Framebuffer::handle() const noexcept
		{
			return this->_fbo;
		}

		const GraphicsFramebufferDesc&
		GL45Framebuffer::getFramebufferDesc() const noexcept
		{
